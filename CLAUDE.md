...
- **FIT upload** (`src/lib/fit.ts`): fit-file-parser returns raw semicircles; convert with `deg = semi * 180 / 2^31`. Routes are downsampled to ≤500 points, stored as JSON on the activity, rendered by `route-svg` as a plain SVG polyline (no map library, deliberate).
